...
// Unique DOM id derived from the full field path (avoids id collisions when the
// same repeater field appears inside multiple sibling parent items).
$fieldId = trim(str_replace(['[', ']'], ['-', ''], $fieldName), '-');
...
<div class="container-builder" id="container-{{ $field->name }}">
                    ...
                    <div class="section-fields px-4 py-4 space-y-3 border-t border-gray-100">
                        @if($layoutName && $layoutDef)
                            @foreach($layoutDef['fields'] as $fieldDef)
                                ...
                                @endphp
                                <div class="section-field group relative">
                                    <button type="button" onclick="removeSectionField(this)" class="absolute top-0 right-0 text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity z-10" title="Remove Field">
                                        <i class="icon-[tabler--x] text-xs"></i>
                                    </button>
                                    @if($fType !== 'toggle')
                                        <label class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5 pr-5">{{ $fLabel }}</label>
                                    @endif
            ...
            <div id="add-menu-{{ $field->name }}" class="hidden absolute left-0 bottom-full mb-2 w-80 bg-white rounded-xl shadow-xl border border-gray-200 z-50 overflow-hidden">
                ...
                <div class="p-2 max-h-80 overflow-y-auto">
                    @foreach($layouts as $layout)
                        @php
                            $icon = $layoutIcons[$layout['name']] ?? 'fa-puzzle-piece';
    ...
    @elseif($isMultiple)
        ...
        <div class="repeater-wrapper border border-gray-200 rounded-lg bg-gray-50 p-3" id="repeater-{{ $fieldId }}">
            <div class="repeater-items space-y-2" id="repeater-items-{{ $fieldId }}">
                @forelse($containerData as $itemIdx => $item)
                    <div class="repeater-item bg-white border border-gray-200 rounded-md p-3 space-y-3" data-index="{{ $itemIdx }}">
                        <div class="flex items-center justify-between">
                            <span class="text-xs text-gray-400 font-medium">#{{ (int) $itemIdx + 1 }}</span>
                            <button type="button" onclick="removeRepeaterItem(this, '{{ $fieldId }}')" class="text-gray-400 hover:text-red-500 transition-colors" title="Remove">
                                <i class="icon-[tabler--trash] text-xs"></i>
                            </button>
                        </div>
                ...
                @empty
                    <div id="repeater-empty-{{ $fieldId }}" class="text-center py-4 text-gray-400 text-xs">
            ...
            <template id="repeater-template-{{ $fieldId }}">
                <div class="repeater-item bg-white border border-gray-200 rounded-md p-3 space-y-3" data-index="__IDX__">
                    ...
                    @foreach($children as $childField)
                        @if($childField->type === 'container')
                            @include('pages.contententry.partials.container-field', ['field' => $childField, 'value' => $childField->default_value, 'namePrefix' => $fieldName . '[__IDX__]'])
                        @else
                            @include('pages.contententry.partials.basic-field', ['field' => $childField, 'value' => $childField->default_value, 'namePrefix' => $fieldName . '[__IDX__]'])
                        @endif
                    @endforeach
                </div>
            </template>
            ...
            <div class="mt-3 text-right">
                <button type="button" onclick="addRepeaterItemFromServer('{{ $fieldId }}')" class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium text-blue-600 bg-blue-50 rounded-md hover:bg-blue-100 transition-colors border border-blue-200">
        ...
        <div class="single-container border border-gray-200 rounded-lg bg-white p-4 space-y-3">
            @if($children->isEmpty())
                <div class="text-center py-6 text-gray-400">
                    ...
                    <p class="text-xs mt-1">
                        <a href="{{ route('custom-field.getUpdate', $field->id) }}" class="text-blue-500 hover:text-blue-700 underline">
                            Add child fields
                        </a>
            ...
            @else
                @foreach($children as $childField)
                    ...
                    @endphp
                    @if($childField->type === 'container')
                        @include('pages.contententry.partials.container-field', ['field' => $childField, 'value' => $childValue, 'namePrefix' => $fieldName])
                    @else
                        @include('pages.contententry.partials.basic-field', ['field' => $childField, 'value' => $childValue, 'namePrefix' => $fieldName])
                    @endif
                @endforeach
            @endif
        </div>
