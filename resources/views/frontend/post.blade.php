...
@section('content')
    <article class="max-w-4xl mx-auto px-8 py-24">
        ...
        <header class="mb-8">
            @php
                $categories = $entry->category_ids ?? [];
                $categoryNames = \App\Models\Category::whereIn('id', $categories)->get();
                $tags = $entry->tag_ids ?? [];
                $tagNames = \App\Models\Tag::whereIn('id', $tags)->get();
            @endphp
        ...
        @if(!empty($entry->featured_image))
            <figure class="mb-8">
                <img src="{{ $entry->featured_image }}" alt="{{ $entry->title }}" class="w-full h-auto rounded-2xl shadow-lg" />
            </figure>
        @endif
